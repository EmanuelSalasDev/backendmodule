...
router.patch('/:idKoder', async (request, response) =>{
    /*
    0.-leer el archivo fs.promise.readFi
    1.- Cual es el Id del koder a actualizar
    2.- buscar al koder en la lista y actualizar al koder
    3.- actualizar el archivo con el koder actualizado
    4.-reponse*/
    const dataFile = await fs.promises.readFile('./kodemia.json', 'utf8')
    const json = JSON.parse(dataFile)
    const { idKoder } = request.params
    const koderData = request.body
    let koderUpdate = json.koders.filter(koder => koder.id !== parseInt(idKoder))
    koderUpdate.push(koderData)
    json.koders = koderUpdate
    ...
    response.json({
        success: true,
        message: 'Actualizacion de Koder Exitosa! '
    })
})
